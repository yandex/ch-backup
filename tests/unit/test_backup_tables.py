...
@pytest.mark.parametrize('mtime, backups_expected', [([1689000195.8, 1689000195.8], 1),
                                                     ([1689000195.8, 1689000200.1], 0)])
def test_backup_table_skipping_if_metadata_updated_during_backup(mtime: List[float], backups_expected: int) -> None:
    table_name = 'table1'
    db_name = 'db1'
    creation_statement = f"ATTACH TABLE db1.table1 UUID '{UUID}' (date Date) ENGINE = MergeTree();"
    ...
    backup_meta = BackupMetadata(name='20181017T210300',
    ...
    clickhouse_ctl_mock.get_tables.return_value = [
        Table(db_name, table_name, 'MergeTree', [], [], '/var/lib/clickhouse/metadata/db1/table1.sql', '', UUID),
    ]
    ...
    # Backup table
    with patch('os.path.getmtime', side_effect=mtime), patch('ch_backup.logic.table.Path', read_bytes=read_bytes_mock):
        table_backup.backup(context, [db], {db_name: [table_name]}, dedup_info, schema_only=False)
