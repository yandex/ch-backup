...
from ch_backup.backup.metadata import (AccessControlMetadata, BackupMetadata, BackupState, BackupStorageFormat)
...
class TestAccessControlMetadata:
    ...
    def test_load(self, data: dict) -> None:
        ...
        assert acl.backup_format == BackupStorageFormat(data.get('backup_format', 'tar'))

    def test_dump(self) -> None:
        acl = AccessControlMetadata(acl_ids=['id_1'],
                                    acl_meta={'0': {
                                        'name': 'acl_1',
                                        'char': 'U',
                                    }},
