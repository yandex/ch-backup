...
def startup_containers(context: ContextT) -> None:
    ...
    for feature_flag in context.feature_flags:
        command += f" --profile {feature_flag}"
...
@utils.env_stage("create", fail=True)
def create_configs(context: ContextT) -> None:
    """
    Generate config files (docker-compose.yml and .env).
    """
    compose_config = _generate_compose_config(context)
    _write_compose_config(context, compose_config)

    env_config = _generate_env_config(context)
    _write_env_config(context, env_config)
...
def _generate_service_config(
    ...
    service = {
        ...
        "profiles": instance_config.get("profiles", []),
    }
...
def _generate_env_config(context: ContextT) -> dict:
    env_config = {}

    for feature_flag in context.feature_flags:
        env_config[f"{feature_flag.upper()}_ENABLED"] = "1"

    return env_config


def _write_env_config(context: ContextT, env_config: dict) -> None:
    config_path = os.path.join(context.conf["staging_dir"], ".env")
    with open(config_path, "w", encoding="utf-8") as file:
