Feature: Backup and restore functionality of access control entities
  ...
  @require_version_22.3
  Scenario: from replicated to local storage
  ...
  @require_version_22.3
  Scenario: from replicated to replicated storage
    Given we have dirty enabled replicated access on clickhouse01 with restart
    And a working clickhouse on clickhouse01
    And we have dirty enabled replicated access on clickhouse02 with restart
    And a working clickhouse on clickhouse02
    And we have executed queries on clickhouse01
    """
    CREATE USER test_user IDENTIFIED WITH plaintext_password BY 'password';
