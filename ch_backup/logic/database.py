...
from ch_backup.clickhouse.schema import (rewrite_database_schema, to_attach_query, to_create_query)
...
class DatabaseBackup(BackupManager):
    ...
    def restore(context: BackupContext, databases: Dict[str, Database]) -> None:
        ...
        present_databases = [db.name for db in context.ch_ctl.get_databases()]

        databases_to_restore: Dict[str, Database] = {
            name: db
            for name, db in databases.items() if not db.has_embedded_metadata() and name not in present_databases
        }
