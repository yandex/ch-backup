...
def fix_s3_oplog(cloud_storage_config: Dict[str, Any],
    ...
    if cloud_storage_source_path is None:
        cloud_storage_source_path = _get_shard_path(cluster_id=source_cluster_id, shard_id=shard)

    engine = S3StorageEngine(cloud_storage_config)
    client = engine.get_client()
    paginator = client.get_paginator('list_objects')
    list_object_kwargs = dict(Bucket=cloud_storage_source_bucket,
    ...
    logging.info(f'Fix S3 OpLog: found {"and fixed " if not dryrun else ""}%d collisions', collision_counter)
