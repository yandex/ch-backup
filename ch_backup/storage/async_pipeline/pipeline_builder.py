...
class PipelineBuilder:
    ...
    def build_read_files_tarball_stage(self, dir_path: Path, file_relative_paths: List[Path]) -> 'PipelineBuilder':
        """
    ...
    def build_uploading_stage(self, remote_path: str, source_size: int) -> 'PipelineBuilder':
        """
    ...
    def build_delete_multiple_storage_stage(self, remote_paths: Sequence[str]) -> 'PipelineBuilder':
        """
        ...
        self.append(thread_map(DeleteMultipleStorageStage(stage_config, remote_paths, storage)))
        return self
    ...
    def pipeline(self) -> PypelnStage:
        ...
        if len(self._stages) == 0:
            raise RuntimeError('Pipeline must not be empty. Please build at least one stage')
